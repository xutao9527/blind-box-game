...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/csgoRollUser/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/csgoRollUser/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/csgoRollUser/save', data)
    if (apiRet.ok) {
