...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/csgoBattleRoomBox/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/csgoBattleRoomBox/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/csgoBattleRoomBox/save', data)
    if (apiRet.ok) {
