<template>
  <el-container class="bbg-table">
    ...
    <el-main class="bbg-table-main">
        ...
        <el-table-column prop="isWin" label="是否赢家">
        <template #default="scope">
            {{scope.row.isWin?'启用':'停用'}}
        </template>
        </el-table-column>
...
const remove = async (row) => {
  const apiRet = await http.get(`/csgoBattleRoomUser/remove/${row.id}`)
  if (apiRet.ok) {
...
const tableProps = reactive({
  ...
  fetchData: async () => {
    const apiRet = await http.post('/csgoBattleRoomUser/page', tableProps.reqParams)
    if (apiRet.ok) {
