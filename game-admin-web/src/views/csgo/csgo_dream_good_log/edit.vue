        ...
        <el-row>
          <el-col :offset="7" :span="8">
            <el-form label-position="right" label-width="120">
              ...
              <el-form-item label="追梦金额">
                <el-input v-model="data.dreamPrice"/>
              </el-form-item>
              <el-form-item label="追梦概率">
                <el-input v-model="data.dreamGoodProbability"/>
              </el-form-item>
              <el-form-item label="追梦时间">
                <el-input v-model="data.dreamGoodTime"/>
              </el-form-item>
              <el-form-item label="追梦成功">
                <el-switch v-model="data.dreamIsWin"/>
              </el-form-item>
...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/csgoDreamGoodLog/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/csgoDreamGoodLog/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/csgoDreamGoodLog/save', data)
    if (apiRet.ok) {
