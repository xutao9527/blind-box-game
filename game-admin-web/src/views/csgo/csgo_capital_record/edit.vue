          ...
          <el-col :offset="7" :span="8">
            <el-form label-position="right" label-width="120">
              ...
              <el-form-item label="变更金额">
                <el-input v-model="data.changeMoney"/>
              </el-form-item>
              <el-form-item label="变更前金额">
                <el-input v-model="data.beforeMoney"/>
              </el-form-item>
              <el-form-item label="变更后金额">
                <el-input v-model="data.afterMoney"/>
              </el-form-item>
...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/csgoCapitalRecord/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/csgoCapitalRecord/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/csgoCapitalRecord/save', data)
    if (apiRet.ok) {
