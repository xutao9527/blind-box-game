          ...
          <el-col :offset="8" :span="8">
            <el-form label-position="right" label-width="auto">
              ...
              <el-form-item label="角色描写">
                <el-input type="textarea" v-model="data.remark"/>
              </el-form-item>
...
const submitText = computed(()=>{
  return data.id ? '修改':'新增'
})

const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/sysRole/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/sysRole/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/sysRole/save', data)
    if (apiRet.ok) {
