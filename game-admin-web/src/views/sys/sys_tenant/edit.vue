...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/sysTenant/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/sysTenant/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/sysTenant/save', data)
    if (apiRet.ok) {
