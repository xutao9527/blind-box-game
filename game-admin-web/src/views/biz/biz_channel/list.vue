  ...
  <el-container class="bbg-table">
    ...
    <el-main class="bbg-table-main">
      <el-table class="bbg-table-main"
        ...
        <el-table-column prop="adminId" label="所属管理"/>
        <el-table-column prop="channelName" label="渠道名称"/>
        <el-table-column prop="channelDomain" label="渠道域名"/>
        <el-table-column prop="channelCode" label="渠道码"/>
        <el-table-column prop="channelRemark" label="渠道描述"/>
...
const remove = async (row) => {
  ElMessageBox.confirm("确认删除?", "提示", {
    ...
    const apiRet = await http.get(`/bizChannel/remove/${row.id}`)
    if (apiRet.ok) {
...
const tableProps = reactive({
  ...
  fetchData: async () => {
    const apiRet = await http.post('/bizChannel/page', tableProps.reqParams)
    if (apiRet.ok) {
