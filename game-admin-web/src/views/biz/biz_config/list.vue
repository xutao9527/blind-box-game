  ...
  <el-container class="bbg-table">
    ...
    <el-main class="bbg-table-main">
      <el-table class="bbg-table-main"  size="small"
        ...
        <el-table-column prop="value" label="配置值"  width="220px"/>
...
const remove = async (row) => {
  const apiRet = await http.get(`/bizConfig/remove/${row.id}`)
  if (apiRet.ok) {
...
const tableProps = reactive({
  ...
  fetchData: async () => {
    const apiRet = await http.post('/bizConfig/page', tableProps.reqParams)
    if (apiRet.ok) {
