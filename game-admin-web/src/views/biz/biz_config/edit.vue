...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/bizConfig/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/bizConfig/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/bizConfig/save', data)
    if (apiRet.ok) {
