...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/bizDict/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/bizDict/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/bizDict/save', data)
    if (apiRet.ok) {
