          ...
          <el-col :offset="7" :span="8">
            <el-form label-position="right" label-width="120">
              <el-form-item label="字典编号">
                <el-input v-model="data.dictId"/>
              </el-form-item>
              <el-form-item label="名称">
                <el-input v-model="data.label"/>
              </el-form-item>
              <el-form-item label="别名">
                <el-input v-model="data.labelAlias"/>
              </el-form-item>
...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/bizDictDetail/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/bizDictDetail/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/bizDictDetail/save', data)
    if (apiRet.ok) {
