        ...
        <el-row>
          <el-col :offset="7" :span="8">
            <el-form label-position="right" label-width="120">
              <el-form-item label="游戏箱子">
                <el-input v-model="data.boxId"/>
              </el-form-item>
              <el-form-item label="游戏商品">
...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/csgoBoxGoods/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/csgoBoxGoods/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/csgoBoxGoods/save', data)
    if (apiRet.ok) {
