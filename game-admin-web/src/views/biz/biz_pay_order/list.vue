  ...
  <el-container class="bbg-table">
    ...
    <el-main class="bbg-table-main">
      <el-table class="bbg-table-main"
        ...
        <el-table-column prop="payNo" label="支付订单号"/>
        <el-table-column prop="payPlatformId" label="支付平台编号"/>
        <el-table-column prop="payCurrency" label="支付币种"/>
        <el-table-column prop="payMoney" label="支付金额"/>
        <el-table-column prop="callbackTime" label="回调时间"/>
...
const remove = async (row) => {
  ElMessageBox.confirm("确认删除?", "提示", {
    ...
    const apiRet = await http.get(`/bizPayOrder/remove/${row.id}`)
    if (apiRet.ok) {
...
const tableProps = reactive({
  ...
  fetchData: async () => {
    const apiRet = await http.post('/bizPayOrder/page', tableProps.reqParams)
    if (apiRet.ok) {
