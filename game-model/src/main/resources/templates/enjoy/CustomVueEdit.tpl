  ...
  <el-container class="bbg-form">
    <el-header class="bbg-form-header">
      <el-text tag="b" size="large" type="primary">
        <h3>{{ submitText }}-#(table.getComment())</h3>
      </el-text>
    </el-header>
        ...
        <el-row>
          <el-col :offset="7" :span="8">
            <el-form label-position="right" label-width="120">
              ...
              #if(column.property != "id" && column.property != "createTime" && column.property != "updateTime" && column.property != "editTime")
              #if(column.propertySimpleType == "Boolean")
              <el-form-item label="#(column.comment)">
                <el-switch v-model="data.#(column.property)"/>
              </el-form-item>
              #else
              <el-form-item label="#(column.comment)">
                <el-input v-model="data.#(column.property)"/>
              </el-form-item>
              #end
              #end
              #end
...
const toEdit = async (id) => {
  if (id) {
    const apiRet = await http.get(`/#(entityVarName)/getInfo/${id}`);
    if (apiRet.ok) {
...
const submit = async () => {
  if (data.id) {
    const apiRet = await http.post('/#(entityVarName)/update', data)
    if (apiRet.ok) {
      backList(true)
    }
  } else {
    const apiRet = await http.post('/#(entityVarName)/save', data)
    if (apiRet.ok) {
